...
exports.clearChat = async (conn, chatId) => {
    try {
        await conn.chatModify({ clear: { messages: [{ id: 'all' }] } }, chatId);
        await conn.deleteChat(chatId); // Menghapus chat dari semua perangkat bot
        console.log(`Semua chat di ${chatId} telah dihapus dari semua perangkat bot.`);
    } catch (error) {
...
exports.smsg = (conn, m, store) => {
    ...
    // Helper aman akses nested properti
    const safeGet = (obj, path, def = '') =>
        path.split('.').reduce((o, key) => (o && o[key] !== undefined ? o[key] : def), obj)
    ...
    if (m.message) {
        ...
        // FIXED: gunakan safeGet agar tahan error
        m.body =
            m.message?.conversation ||
            safeGet(m, 'msg.caption') ||
            safeGet(m, 'msg.text') ||
            (m.mtype === 'listResponseMessage' && safeGet(m, 'msg.singleSelectReply.selectedRowId')) ||
            (m.mtype === 'buttonsResponseMessage' && safeGet(m, 'msg.selectedButtonId')) ||
            (m.mtype === 'viewOnceMessage' && safeGet(m, 'msg.caption')) ||
            m.text || ''
